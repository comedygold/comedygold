...
uint256 hashGenesisBlock("0x04374ca59a0c8debeb9722a03cdd844f2588d008d2055ebdca365215a80d8706");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ComedyGold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ComedyGold Signed Message:\n";
...
{
    // ComedyGold: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // ComedyGold: every 4 hours
static const int64 nTargetSpacing = 60; // ComedyGold: 1 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ComedyGold: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // ComedyGold: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x04374ca59a0c8debeb9722a03cdd844f2588d008d2055ebdca365215a80d8706");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "you have a carrot in your bottom";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396114821;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3805363;
        ...
        assert(block.hashMerkleRoot == uint256("0xc7e3ce2551ffd0849ae46a7062d6ff4b24c82a62194ed0f151cd7faf9ef2b87a"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ComedyGoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ComedyGoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ComedyGoldMiner(CWallet *pwallet)
{
    printf("ComedyGoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("comedygoldcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ComedyGoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ComedyGoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ComedyGoldMiner, pwallet));
}
