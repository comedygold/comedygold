...
#else
    const char* pszModule = "comedygoldcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ComedyGold";
#else
    fs::path pathRet;
    ...
    return pathRet / "ComedyGold";
#else
    // Unix
    return pathRet / ".comedygoldcoin";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "comedygoldcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "comedygoldcoind.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // ComedyGold: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ComedyGold will not work properly.");
